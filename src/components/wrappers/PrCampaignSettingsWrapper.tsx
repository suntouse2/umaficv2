import PrCampaignService from '@api/http/services/campaigns/pr/PrCampaignService'
import { transformStoreToSettings } from '@helpers/prCampaignSettingsTransformer'
...
type PrCampaignSettingsWrapperProps = {
	campaignId: number | null
} & PropsWithChildren

export default function PrCampaignSettingsWrapper({
	campaignId,
	children,
}: PrCampaignSettingsWrapperProps) {
	const resetSettings = usePrCampaignSettingsStore(state => state.resetSettings)
	const setSettings = usePrCampaignSettingsStore(state => state.setSettings)
	const currentCampaignId = usePrCampaignSettingsStore(state => state.campaignId)
	const getStore = usePrCampaignSettingsStore(state => state.getStore)
	const setStore = usePrCampaignSettingsStore(state => state.setStore)
	const [cachedStore, setCachedStore] = useState<ReturnType<typeof getStore> | null>(null)
	const localStorageCacheName = (id: number | null) => 'prCampaign-' + id
	const [loading, setLoading] = useState<boolean>(true)
	const fetchSettings = useCallback(
		async (campaignId: number) => {
			const { data } = await PrCampaignService.getPrCampaign(campaignId)
