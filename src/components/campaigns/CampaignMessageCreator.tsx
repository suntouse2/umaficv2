...
export default function CampaignMessageCreator({
	data: { order, keywords, message },
	maxMsgLength,
	updateMessage,
	onClose,
	filter_type,
}: CampaignMessageCreatorProps) {
	const [newOrder, setNewOrder] = useState<number | undefined>(
		filter_type === 'order' ? order : undefined
	)
	const [newKeyword, setNewKeyword] = useState<Set<string>>(
		filter_type === 'keyword' ? new Set([...(keywords || [])]) : new Set()
	)
	const [newMedia, setNewMedia] = useState<TFunnelMessage['media']>(
		message.media
	)
		...
		<div className='p-2 flex flex-col gap-3'>
			...
			{filter_type === 'order' && (
				<Input
					...
					onChange={value => setNewOrder(Math.max(Number(value), 2))}
				/>
			)}
				...
				<>
					...
					{newKeyword.size > 0 && (
						<div>
							...
							<TagList
								tagClassName='!bg-softgray'
								className='mt-0'
								editable
								value={new Set(newKeyword)}
								onChange={tags => setNewKeyword(tags)}
							/>
						</div>
					)}
				</>
			...
			<textarea
				...
				maxLength={maxMsgLength ?? 4096}
				value={newMessage}
				onChange={e => setNewMessage(e.target.value)}
				className='text-sm p-2 outline-none w-full min-h-[100px] !bg-softgray rounded-lg'
