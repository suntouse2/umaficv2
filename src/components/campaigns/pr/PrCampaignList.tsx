import { useFetchPrCampaigns } from '@api/queries'
import { CircularProgress } from '@mui/material'
import { memo, useEffect, useRef } from 'react'
import { useInView } from 'react-intersection-observer'
import PrCampaignCard from './PrCampaignCard'

export default memo(function DirectCampaignsList() {
	const { data, isPending, fetchNextPage } = useFetchPrCampaigns()
	...
	return (
		<ul className='flex flex-col p-0 pt-2 md:p-4 md:pt-3 sm:grid grid-cols-auto-fit-450 gap-2'>
			...
			{campaigns.map(campaign => (
				<li key={campaign.id}>
					<PrCampaignCard campaign={campaign} />
				</li>
			))}
