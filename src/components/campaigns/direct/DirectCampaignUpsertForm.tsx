...
export default function CampaignUpsertForm({
	...
	const tabs = [
		...
		{ name: 'Таргет', disabled: Boolean(errors.settings?.profile) },
		{
			name: 'Сообщения',
			disabled:
				Boolean(errors.settings?.profile) || Boolean(errors.settings?.target),
		},
		{
			name: 'Запуск',
			disabled:
				Boolean(errors.settings?.profile) ||
				Boolean(errors.temporary?.first_message) ||
				(watch('settings.auto_reply.use_assistant')
					? Boolean(errors.settings?.auto_reply?.assistant?.description)
					: false),
		},
	]
	...
	const handleSubmit = async (
		...
		if (Number(getValues('budget_limit')) < 8)
			return toast.error('Бюджет кампании должен быть больше или равен 8₽')
	...
	return (
		<>
			<form
				...
				className='mx-auto w-full h-full max-w-[600px]'
			>
				...
				<div>
					<Tabs
						...
						value={currentStep}
						onChange={(_e, v) => setCurrentStep(v)}
						variant='scrollable'
						scrollButtons='auto'
						...
						<Bubble className='mt-4 relative'>
							<div className='grid grid-cols-1 sm:grid-cols-2 p-4 gap-5'>
								<div className='flex flex-col items-center  gap-5'>
									...
									<Controller
										rules={{
											...
											maxLength: {
												value: 64,
												message: 'Имя должно быть до 64 символов',
											},
										}}
										defaultValue={''}
										control={control}
										name='settings.profile.first_name'
										render={({
									...
									<Controller
										rules={{
											maxLength: {
												value: 64,
												message: 'Фамилия должна быть до 64 символов',
											},
										}}
										defaultValue={''}
										control={control}
										name='settings.profile.last_name'
										render={({
											field: { onChange, value },
											fieldState: { error },
										}) => (
									...
									<Controller
										rules={{
											maxLength: {
												value: 70,
												message: 'Обо мне должно быть до 70 символов',
											},
										}}
										defaultValue={''}
										control={control}
								...
								<div className='hidden sm:block'>
									<p className='text-sm'>
										1. Нажмите на кнопку "Загрузить фото" и выберите
										изображение, которое вы хотели бы использовать для профиля
										бота. <br />
										<br /> 2. Введите имя и фамилию, которые будут отображаться
										на странице бота.
										<br />
										<br /> 3. Напишите краткую информацию в графу "О себе". Это
										поможет создать впечатление, что страница принадлежит
										реальному человеку.
										<br />
										<br /> 4. Обратите внимание, что указание сферы интересов не
										обязательно, но если вы хотите, чтобы ваш бот выглядел
										максимально натурально, можете добавить информацию о его
										увлечениях или интересах.
									</p>
								</div>
							</div>
						</Bubble>
					...
					<div className={`block ${currentStep !== 1 && 'hidden'}`}>
						<Bubble className='relative mt-4'>
							...
							<p className='text-sm mt-2 mb-2'>
								Выберите нужные регионы и увеличьте эффективность вашей рекламы!
								Настройте точечную рекламу и достигните своей целевой аудитории
								прямо сейчас.
							</p>
							<Controller
								defaultValue={[]}
								control={control}
								name='settings.target.geo.language'
								render={({ field: { value, onChange } }) => (
									<SelectList
										...
										label='Добавить язык'
										options={new Map(Object.entries(languagesData ?? []))}
									/>
								)}
							/>
							<Controller
								defaultValue={[]}
								control={control}
								name='settings.target.geo.country'
								render={({ field: { value, onChange } }) => (
									<SelectList
										value={new Set(value)}
										onChange={v => onChange(Array.from(v))}
										...
										options={new Map(Object.entries(countriesData ?? []))}
									/>
								)}
							/>
							<Controller
								defaultValue={[]}
								control={control}
								name='settings.target.geo.region'
								render={({ field: { value, onChange } }) => (
									<SelectList
										value={new Set(value)}
										onChange={v => onChange(Array.from(v))}
										...
										options={new Map(Object.entries(regionsData ?? []))}
									/>
								)}
							/>
							<Controller
								...
								render={({ field: { value, onChange } }) => (
									<SelectList
										...
										options={new Map(Object.entries(citiesData ?? []))}
									/>
								)}
							/>
						</Bubble>
					...
					<div className={`block ${currentStep !== 2 && 'hidden'}`}>
						<Bubble className='relative mt-4'>
							...
							<Controller
								rules={{ validate: e => e && e.length > 0 }}
								defaultValue={[]}
								control={control}
								name='settings.target.search.include'
								render={({ field: { value, onChange } }) => (
									<KeywordManager
										value={new Set(value)}
										onChange={v => onChange(Array.from(v))}
						...
						<Bubble className='mt-4 relative'>
							...
							<p className='text-sm mt-2'>
								Фразы, которые система будет исключать из поиска в чатах
								Telegram.
							</p>
							<Controller
								defaultValue={[]}
								control={control}
								name='settings.target.search.exclude'
								render={({ field: { value, onChange } }) => (
									<KeywordManager
										value={new Set(value)}
										onChange={v => onChange(Array.from(v))}
						...
						<Bubble className='mt-4'>
							<p className='text-sm'>
								Перед запуском Umafic Таргет рекомендуем проверить настройки
								поисковых фраз. <br />
								<br /> Для этого используем кнопку "Проверить Настройки".
								<br />
								<br /> При этом система в режиме реального времени покажет, на
								какие сообщения будет реагировать. Благодаря этому инструменту
								вы сможете решить, какие фразы нужно добавить или исключить из
								"Поисковых фраз", а также "Минус-фраз". <br />
								<br /> Для добавления, нажмите на слово в найденном сообщении,
								при необходимости измените его и добавьте в "Поисковые фразы"
								или "Минус-фразы"{' '}
							</p>
							{watch('settings.target.search.include')?.length > 0 ? (
								...
								<p className='text-base mt-4 text-negative'>
									Чтобы проверить настройки, добавьте минимум одно ключевое
									слово.
								</p>
							)}
						</Bubble>
					...
					<div className={`block ${currentStep !== 3 && 'hidden'}`}>
						...
						<p className='mt-2 mb-2 text-sm'>
							Вы можете выбрать только один из двух режимов. По порядку или по
							ключевым словам. На данный момент выбрано:{' '}
						...
						<div
							className={`block ${
								watch('settings.auto_reply.funnel.funnel_type') == 'keyword' &&
								'hidden'
							}`}
						>
							<Bubble className='mt-2 relative'>
								...
								<p className='text-sm mb-4'>
									Сообщение, которое будет отправлено в чат или в личные
									сообщения первым.
								</p>
								<Controller
									rules={{
										validate: {
											...
											lessThan5: e =>
												(e && e[0] && e[0].messages.length >= 5) ||
												'Должно быть минимум 5 вариантов первого сообщения',
											hasLink: e =>
												(e &&
													e[0].messages.every(
														msg => !containsLink(msg.message)
													)) ||
												'В первом сообщение не должно быть ссылок',
										},
									}}
									defaultValue={[]}
									control={control}
						...
						>
							...
							<Bubble className='mt-2 relative'>
								...
								<h2 className='text-lg font-bold'>
									Настройка диалога по ключевым словам
								</h2>
								<p className='text-sm mb-4'>
									Сообщения, которые будут отправляться, если пользователь
									продолжил общение.
								</p>
								<Controller
									defaultValue={[]}
									control={control}
									name='settings.auto_reply.funnel.keyword'
									render={({ field: { value, onChange } }) => (
										<CampaignMessageManager
											filter_type='keyword'
											value={value}
							...
							<Bubble className='mt-2 relative'>
								...
								<h2 className='text-lg font-bold'>
									Настройка диалога на любые фразы
								</h2>
								<p className='text-sm mb-4'>
						...
						<Bubble className='mt-2 relative'>
							...
							<div
								className={`block ${
									watch('settings.auto_reply.use_assistant') == false &&
									'hidden'
								}`}
							>
								<Controller
									...
									render={({
										...
										return (
											<div className='mt-2 w-full'>
												<p>
													Информация для ИИ (контакты, детали, описание
													продукта):
												</p>
												<TextArea
													maxLength={2048}
					...
					<div className={`block ${currentStep !== 4 && 'hidden'}`}>
						<Bubble>
							...
							<Controller
								...
								render={({ field: { onChange, value } }) => (
									<Input
										...
										onBlur={e => onChange(parseBudget(e.target.value))}
										value={value}
										onChange={onChange}
							...
							<b className='mt-2 text-sm'>
								...
								<span className='text-secondary'>
									{formatBalance(
										statsData?.data.recommended_daily_budget_limit
									)}
								</span>
							</b>
						</Bubble>
