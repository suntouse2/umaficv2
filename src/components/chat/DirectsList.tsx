...
export default function DirectsList() {
	const { isFavorite, directs, setIsFavorite, fetchNextDirects, campaignId, openDirect } =
		useChat()

	const navigate = useNavigate()

	const handleDirectClick = ({ id }: TChatDirect) => {
		if (directs.find((direct: { id: number }) => direct.id == id)?.is_open == false) {
			openDirect(id)
		}
	...
	return (
		<div className='flex flex-col border-r-[1px] h-full overflow-hidden border-softgray w-full min-w-[400px] lg:max-w-[400px] '>
			...
			<Virtuoso
				...
				data={directs}
				itemContent={(_index, direct) => (
					<Direct onClick={() => handleDirectClick(direct)} direct={direct} />
				)}
				endReached={() => {
					fetchNextDirects()
				}}
