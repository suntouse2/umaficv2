...
export function ChatProvider({
		...
		ws.onmessage = event => {
			...
			if (data) {
				...
				setMessagesCache(currentMessages => {
					...
					if (wsMsg.is_self) {
						const indexOfTempMessage = messages.findIndex(
							tempMessage => tempMessage.catch_slug === wsMsg.catch_slug
						)
						...
						if (indexOfTempMessage !== -1 && wsMsg.catch_slug !== null) {
							...
							return {
								...currentMessages,
								[wsMsg.direct_id]: [...messages, wsMsg],
							}
						}
					} else {
						return {
	...
	const openDirect = useCallback(async (direct_id: number) => {
		try {
			...
			setDirects(directs =>
				directs.map(direct =>
					direct.id === direct_id ? { ...direct, is_open: true } : direct
				)
			)
		} catch {
			//
			...
			try {
				...
				if (currentDirect && currentDirect.id === direct_id) {
					setCurrentDirect(direct =>
						direct ? { ...direct, is_favorite: isFavorite } : direct
					)
				}
				setDirects(directs =>
					directs.map(direct =>
	...
	const readMessage = useCallback(async (direct_id: number, msg_id: number) => {
		try {
			...
			setMessagesCache(msgs => ({
				...msgs,
				[direct_id]: [
					...msgs[direct_id].map(msg =>
						msg.id == msg_id ? { ...msg, is_read: true } : msg
					),
				],
			}))
			setDirects(directs =>
				directs.map(direct =>
					direct.id === direct_id ? { ...direct, unread_count: 0 } : direct
				)
			)
		} catch {
			//
